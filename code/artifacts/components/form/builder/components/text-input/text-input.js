...
export class TextInput extends Component {
    ...
    render() {
        const { t, input, groupName, metadata, onSubmit, meta: { touched, error, active }, } = this.props;
        ...
        return (<View style={{ paddingTop: active ? 5 : 0, margin: 5 }}>
        <Item error={touched && !!error} success={touched && !!!error} inlineLabel={metadata.labelType === "inline"} floatingLabel={!metadata.labelType || metadata.labelType === "floating"} stackedLabel={metadata.labelType === "stacked"} placeholderLabel underline={!metadata.textboxType || metadata.textboxType === "underlined"} style={{ flexDirection: metadata.labelType !== "inline" ? "row-reverse" : "row" }}>
          ...
          <NativeBaseInput returnKeyType={metadata.returnKey} placeholder={active ? placeholder : ""} withRef getRef={r => (this.input = r)} keyboardType={metadata.keyboardType || metadata.type === "number" ? "numeric" : "default"} onSubmitEditing={() => (onSubmit ? setTimeout(onSubmit) : null)} secureTextEntry={metadata.type === "password"} {...input}/>
          {touched && !!error ? (<Icon type="FontAwesome" style={styles.errorStyle} name="exclamation-circle"/>) : null}
          {touched && !!!error ? (<Icon type="FontAwesome" style={styles.successStyle} name="check-circle"/>) : null}
        </Item>
    ...
    renderError(errorObject) {
        if (typeof errorObject === "string") {
            ...
            return (<Text style={styles.errorStyle}>{this.props.t(errorObject.text, errorObject.params)}</Text>);
        }
    }
    t(s) {
        if (s) {
